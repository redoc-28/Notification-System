...
class SMSDeliveryLogModel(Base):
    """Model for storing SMS delivery logs"""
    ...
    status = Column(Enum(SMSStatus), nullable=False, index=True)
...
class DatabaseClient:
    ...
    async def log_sms_delivery(self, 
                              log_id: str,
                              notification_id: str,
                              user_id: str,
                              provider: str,
        ...
        Args:
            ...
            user_id: User ID
            provider: SMS provider (twilio, sns, nexmo)
            status: Delivery status
            body: SMS body text
            sender_id: Sender ID or phone number
            sent_at: When the SMS was sent
            delivered_at: When the SMS was delivered
        ...
        try:
            async with async_session() as session:
                ...
                logger.info(f"Logged SMS delivery: {notification_id}, status: {status}")
                return True
                
        except Exception as e:
            logger.error(f"Error logging SMS delivery: {str(e)}")
            return False
            
    async def update_sms_status(self,
                              log_id: str,
                              status: SMSStatus,
                              delivered_at: Optional[datetime] = None,
                              provider_message_id: Optional[str] = None,
        ...
        """
        Update the status of an SMS delivery
        ...
        try:
            async with async_session() as session:
                # Get the log entry
                query = sa.select(SMSDeliveryLogModel).where(SMSDeliveryLogModel.id == log_id)
                result = await session.execute(query)
                log_entry = result.scalar_one_or_none()
                
                if not log_entry:
                    logger.warning(f"SMS log entry not found: {log_id}")
                    return False
                ...
                logger.info(f"Updated SMS status: {log_id}, new status: {status}")
                return True
