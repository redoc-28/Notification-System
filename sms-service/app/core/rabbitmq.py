...
class RabbitMQClient:
    ...
    async def setup_consumer(self, message_handler: Callable[[Dict[str, Any], Dict[str, Any]], Awaitable[bool]]):
        """
        Set up a consumer for the SMS queue
        ...
        queue_args = {
            ...
            "x-dead-letter-routing-key": "notification.sms.failed"
        }
        ...
        await dlq_queue.bind(
            self.dlx_exchange,
            routing_key="notification.sms.failed"
        )
        
        # Start consuming messages
        await sms_queue.consume(self._on_message)
        
        logger.info(f"Started consuming from queue: {self.settings.SMS_QUEUE}")
