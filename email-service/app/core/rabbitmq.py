...
class RabbitMQClient:
    ...
    async def setup_consumer(self, message_handler: Callable[[Dict[str, Any], Dict[str, Any]], Awaitable[bool]]):
        """
        Set up a consumer for the email queue
        ...
        await dlq_queue.bind(
            self.dlx_exchange,
            routing_key="notification.email.failed"
        )
        
        # Start consuming messages
        await email_queue.consume(self._on_message)
        
        logger.info(f"Started consuming from queue: {self.settings.EMAIL_QUEUE}")
